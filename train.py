...
def train(model, device, train_loader, optimizer, epoch):
    ...
    rmse_score = rmse(all_labels, all_predictions)
    mse_score = mse(all_labels, all_predictions)
    pearson_corr = pearson(all_labels, all_predictions)
    spearman_corr = spearman(all_labels, all_predictions)
    ci_score = ci(all_labels, all_predictions)
    r2score = r_2(all_labels, all_predictions)
    ...
    print(f"  Concordance Index (CI): {ci_score:.4f}")
    print(f"  R2_Score: {r2score:.4f}")
    print(f'Average loss: {total_train_loss / len(train_loader):.4f}', flush=True)
...
drug_file = "D:/saved_drug_graph.pickle"
saved_drug_graph = {}
